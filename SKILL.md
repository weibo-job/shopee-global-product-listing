...
5. Only when **no** original-pixel crop can simultaneously be complete and have width/height >= 0.5, call `imagegen` for that screen. Pass every dependent original as `referenced_image_paths`; do not use padding, canvas extension, arbitrary manual stitching, stretching, essential-content cropping, or a text prompt that redraws the product.
...
- Store every persistent artifact produced during an upload task under `/Users/fudasu/Desktop/ai自动化上架/<product-folder>/`.
